    ...
    [SVProgressHUD show];
        ...
        if (self.candID !=nil && self.donorID !=nil) {
            ...
            if ([[NSUserDefaults standardUserDefaults] boolForKey:@"Check"] ){
                ...
                dict=@{@"email":emailString ,@"candid":self.candID,@"donorid":self.donorID,@"pledge":self.callAmountTextField.text,@"ps":self.psCallTextField.text};
                
            }else{
                
                dict=@{@"email":self.emailCallTextField.text ,@"candid":self.candID,@"donorid":self.donorID,@"pledge":self.callAmountTextField.text,@"ps":self.psCallTextField.text};
            }
